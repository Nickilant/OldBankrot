...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="User-Agent браузера (по умолчанию Chromium-подобный)",
    )

    args = parser.parse_args()

    app = QApplication(sys.argv)
    profile = QWebEngineProfile("fedresurs-profile", app)
    ...
    def on_url_changed(url: QUrl):
        print(f"Открыт URL: {url.toString()}")

    view.loadFinished.connect(on_load_finished)
    view.urlChanged.connect(on_url_changed)

    print(f"Используется User-Agent: {args.user_agent}")
